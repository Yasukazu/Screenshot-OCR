...
GLOB_MODE = StrEnum('GLOB', ['NONE', 'GLOB', 'RGLOB'])
...
class MainSettings(AppSettings):
	...
	exclude_area_param_set: set[str] = field(default_factory=set)
...
def main_settings_toml_lines(Settings: Type[Settings]=MainSettings) -> Iterator[str]:
	...
	for line in Binder(Settings()).format_toml_template():
		yield line
...
def load_merged_settings(by_file_settings: MainSettings, main_settings_class=MainSettings, sub_settings_class=MainSettings) -> MainSettings:
	...
	parser.add_arguments(sub_settings_class, dest='settings')
...
if __name__ == '__main__':
	...
	settings = AppSettings().parse_args(argv[1:])
	from simple_parsing import parse as simple_parse
	app_settings: AppSettings = simple_parse(config_class=AppSettings, config_path='app-config.yaml')
	app_settings.app_to_suffixes |= AppSettings().app_to_suffixes
	...
	user_home_dir = Path('~').expanduser()
	main_settings_file = search_settings_file(__file__)
