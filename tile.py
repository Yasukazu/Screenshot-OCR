...
class ArcFileExt(Enum):
	...
	PDF = ('.pdf', {})
from tool_pyocr import AppType, MyOcr
...
from main_my_ocr import run_ocr, save_as_csv, Main

class FunctionItem:
	def __init__(self, title: str, func: Callable, kwargs: dict[str, Any]={}, main_obj: Optional[Main]=None, month=0, app_type=AppType.NUL, tbl_ver=1):
		self.title = title
		self.func = func
		self.kwargs = kwargs
		if main_obj:
			...
			self.main = Main(my_ocr=my_ocr, app_type=app_type, tbl_ver=tbl_ver)

	def exec(self):
		self.func(**self.kwargs)

def get_options(month=0, app_type=AppType.NUL, tbl_ver=1):
		my_ocr = MyOcr(month=month)
		main = Main(my_ocr=my_ocr, app_type=app_type, tbl_ver=tbl_ver)
		return main, [
		FunctionItem('Exit', None, main_obj=main),
		FunctionItem('T run OCR', run_ocr, kwargs={'app_type': AppType.T}, main_obj=main),
		FunctionItem('M run OCR', run_ocr, kwargs={'app_type': AppType.M}, main_obj=main),
...
#        FunctionItem('save MH screenshots as TIFF', save_arc_pages, kwargs={'app_type': AppType.M}),
		FunctionItem('T save_qpng_pages png files into qpng dir.', save_qpng_pages, main_obj=main),
		FunctionItem('M save_qpng_pages png files into qpng dir.', save_qpng_pages, main_obj=main,kwargs={'app_type': AppType.M}),
#        FunctionItem('save_pages_as_TIFF', save_pages_as_tiff),
		FunctionItem('T convert_to_pdf', convert_to_pdf, main_obj=main, kwargs={'layout':PdfLayout.a3lp, 'app_type': AppType.T}),
		FunctionItem('M convert_to_pdf', convert_to_pdf, main_obj=main, kwargs={'layout':PdfLayout.a3lp, 'app_type': AppType.M}),
		FunctionItem('save_as_csv', save_as_csv, main_obj=main, kwargs={}),
	]

def main(get_options_func=get_options, month=0, app_type=AppType.NUL, tbl_ver=1):
	main_obj, options = get_options_func(month=month, app_type=app_type, tbl_ver=tbl_ver)
	if main_obj is None:
		my_ocr = MyOcr(month=month)
		main_obj = Main(my_ocr=my_ocr, app_type=app_type, tbl_ver=tbl_ver)
	choice = None
