...
class MyOcr:
	...
	@classmethod
	def check_date_time(cls, app_type: AppType, txt_lines: Sequence[LineBox]) -> tuple[int, Date|None, Sequence[str]]:    
		match app_type:
			case AppType.M:
				word_match_list: list[PatternMatch] = []
				for n, txt_line in enumerate(txt_lines):
					for word_box in txt_line.word_boxes :
						word = unicodedata.normalize('NFKC', word_box.content)
				...
				if len(word_match_list) > 0:
					...
					for word_match in word_match_list:
						if word_match.pattern == DateTimePattern.DATE:
							...
							hours.append(word_match.match.groups())
					return n, date, hours
			case _:
				raise NotImplementedError("Not implemented yet!")

	@classmethod
	def check_date(cls, app_type: AppType, txt_line_list: Sequence[LineBox|str])->tuple[int, Date|None,Sequence[str]]:
		""" returns (line number, date, hours) """
		if isinstance(txt_line_list[0], LineBox):
			return cls.check_date_time(app_type, txt_line_list)
		else:
			match app_type:
				case AppType.M:
					for n, txt_line in enumerate(txt_line_list):
						joined = ''.join(txt_line.split())
						cntnt = unicodedata.normalize('NFKC', joined)
						if MyOcr.M_DATE_PATT[-1].search(cntnt): # if cntnt.endswith('時間'):
							if (hours:=MyOcr.M_DATE_PATT[0].findall(cntnt)):
								...
								if m_d:
									...
									return n, None, hours
					raise MDateError(f"Could not resolve date! AppType.M txt_lines!:{txt_line_list}")
				case _:
					raise NotImplementedError("Not implemented yet!")
			...
			for app_type in app_type_list:
				for stem in app_type_to_stems[app_type]:
					...
					if app_type == AppType.M:
						n, dt, hrs = self.my_ocr.check_date(app_type=app_type, txt_line_list=txt_lines)
						if dt is None:
							...
							match box_result:
								case Success(value):
									n, dt, hrs = self.my_ocr.check_date(app_type=app_type, txt_line_list=[value])
									if dt is None:
					...
					if one is None:
						...
						cur.execute(insert_sql, (wages, title, file.stem, pickle.dumps(txt_lines), checksum))
						logger.info("INSERT: %s", (app, dt.day, wages, title, file.stem, checksum))
						limit -= 1
						if limit <= 0:
