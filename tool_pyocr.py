...
class MyOcr:
	...
	@classmethod
	def ignore_brackets(cls, s: str):
		return ''.join([c for c in s if unicodedata.category(c)[0] not in ["P", "S", "Z", "C"]])

	@classmethod
	def t_title(cls, txt_lines: Sequence[LineBox], startfrom=0, breakbefore='この店舗の募集状況'):
		# return txt_lines[1].content.replace(' ', '')
		n = -1
		for n, txt_line in enumerate(txt_lines):
			if n < startfrom:
				continue
			if txt_line.content.replace(' ', '').startswith(breakbefore):
				break
		if n < 0 or n >= len(txt_lines) - 1:
			logger.error("No '{}' found in txt_lines!", breakbefore)
			raise ValueError(f"No {breakbefore} found in txt_lines!")
		return ''.join([cls.ignore_brackets(line.content) for line in txt_lines[startfrom:n]])
