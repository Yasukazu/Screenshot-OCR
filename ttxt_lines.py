class TTxtLines:
	...
	def get_title_lines(self, startfrom=0, breakbefore='この店舗の募集状況') -> list[str]:
		n = -1
		for n, txt_line in enumerate(self.txt_lines):
			...
			if txt_line.content.replace(' ', '').startswith(breakbefore):
				break
		if n < 0 or n >= len(self.txt_lines) - 1:
			logger.error("No '{}' found in txt_lines!", breakbefore)
			raise ValueError(f"No {breakbefore} found in txt_lines!")
		return [line.content.replace(' ', '') for line in self.txt_lines[n: breakbefore]]
