...
```text
usage: image_filter.py [-h] [--image_ext IMAGE_EXT [IMAGE_EXT ...]]
                       [--image_dir IMAGE_DIR] [--shot_month SHOT_MONTH]
                       [--app_stem_end APP_STEM_END]
                       [--image_area_param_section_stem IMAGE_AREA_PARAM_SECTION_STEM]
                       [--app_border_ratio [APP_BORDER_RATIO ...]]
                       [--app_suffix] [--app {taimee,mercari}] [--save SAVE]
                       [--nth NTH] [--glob-max GLOB_MAX] [--show]
                       [--make MAKE] [--no-ocr] [--ocr-conf OCR_CONF]
                       [--psm PSM] [--area_param_dir AREA_PARAM_DIR]
                       [--area_param_file AREA_PARAM_FILE]
                       [--ocr_filter_sqlite_db_name OCR_FILTER_SQLITE_DB_NAME]
                       [--data_year DATA_YEAR] [--show_ocr_area]
                       [--exclude_area_param_set [EXCLUDE_AREA_PARAM_SET ...]]
                       [files ...]

positional arguments:
  files                 Image file fullpaths to commit OCR or to get
                        parameters.
...
options:
  ...
  --make MAKE           make a image area param config file from image in TOML
                        format(i.e. this arg. makes not to use param configs
                        in any config file; specify image_area_param values
                        like "--image_area_param heading:0,106,196,-1"
  --no-ocr              Do not execute OCR
  --ocr-conf OCR_CONF   Confidence threshold for OCR
  --psm PSM             PSM value for Tesseract
  --area_param_dir AREA_PARAM_DIR
                        Screenshot image area parameter config file directory
                        [env var: IMAGE_FILTER_AREA_PARAM_DIR]
  --area_param_file AREA_PARAM_FILE
                        ...
                        "heading=[0,106,196,-1]") [env var:
                        IMAGE_FILTER_AREA_PARAM_FILE]
  --ocr_filter_sqlite_db_name OCR_FILTER_SQLITE_DB_NAME
                        SQLite DB file is created under `image_dir`/{yyyy}
                        directory(yyyy is like 2025) [env var:
                        OCR_FILTER_SQLITE_DB_NAME]
  --data_year DATA_YEAR
                        Year for DB data (like 2025) [env var:
                        OCR_FILTER_DATA_YEAR]
  --show_ocr_area       Show every area before commit OCR
  --exclude_area_param_set [EXCLUDE_AREA_PARAM_SET ...]
                        Exclude a set of image area parameter names :
                        {'salary', 'shift', 'breaktime', 'heading', 'paystub'}
                        [env var: IMAGE_FILTER_AREA_PARAM_NAME_EXCLUDE_SET]

Args that start with '--' can also be set in a config file
(/home/yasukazu/github/screen/edge_detect/image-filter.toml or
/home/yasukazu/github/screen/edge_detect/image-filter.ini). Uses multiple
config parser settings (in order):  [1] TOML: Config file syntax is Tom's
Obvious, Minimal Language. See https://github.com/toml-
lang/toml/blob/v0.5.0/README.md for details.  [2] INI: Uses configparser
module to parse an INI file which allows multi-line values. See
https://docs.python.org/3/library/configparser.html for details. This parser
includes support for quoting strings literal as well as python list syntax
evaluation. Alternatively lists can be constructed with a plain multiline
string, each non-empty line will be converted to a list item.   In general,
