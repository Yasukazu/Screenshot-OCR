...
try:
    ...
    try:
        ...
        logger.error(
            "Failed to get '%s' from environment variable or '.env' file:%s",
            APP_TO_SUFFIX_KEY,
            err,
        )
        raise err
...
MAIN_SETTINGS_FILENAME = os_environ.get(
    "IMAGE_FILTER_MAIN_SETTINGS_FILENAME", "image-filter-main-settings.toml"
)

AREA_PARAM_NAME = Enum(
    "AREA_PARAM_NAME", ["HEADING", "SHIFT", "BREAKTIME", "PAYSTUB", "SALARY"]
)
...
from typed_argparser.exceptions import (
    ValidationError,  # ArgumentError, ValidatorInitError,
)
...
def get_app_sym_to_suffix(
    env_var: str = "SCREENSHOT_APP_TO_SUFFIX", load_find_dotenv: str = ".env"
) -> dict[str, str] | None:
    """Get the app symbol to suffix mapping using dotenv('.env' file or environment variables)"""
    try:
        from .app_to_suffix import APP_TO_SUFFIX  # app symbol to number mapping

        return {e.name: e.value for e in APP_TO_SUFFIX}
    except ImportError:
        pass
    if load_find_dotenv:
        from dotenv import find_dotenv, load_dotenv

        _ = load_dotenv(find_dotenv(load_find_dotenv, raise_error_if_not_found=True))
    try:
        screenshot_app_to_suffix = os_environ[env_var]
    except KeyError as e:
        logger.error("No env. var. :%s", env_var)
        raise ValueError(f"No env. var {env_var=} in this environment.") from e
...
class Settings(BaseSettings):
    ...
    app: Optional[str]
    """Application symbol to process OCR from its screenshots: symbols are defined in `app_sym.py`."""
    # , validator=ChoicesValidator([m.name for m in APP_SYM])) # default=None,$

    app_sym_to_suffix: Optional[dict[str, str]] = field(
        default_factory=get_app_sym_to_suffix
    )
    """Application symbol to suffix mapping"""
...
class AppSettings(Settings):
    ...
    stem_delimiter: Optional[str] = argfield(
        default="_",
        help=" Delimiter for splitting screenshot filename stem into 3 parts like:: prefix:'Screenshot', datetime:'yyyy-mm-ddThh:mm:ss', suffix:'com.example.app.name'",
    )
...
class MainSettings(AppSettings):
    ...
    recurse_symlinks: Optional[bool] = argfield(
        default=False, help="Use symbolic links for searching glob pattern"
    )
    case_sensitive: Optional[bool] = argfield(
        default=False,
    ...
    image_area_param_section_stem: str = argfield(
        default="image-area-param",
        help="Image area parameter section/table in image-area-param.ini",
    )
    app_border_ratio: dict[str, str] | None = argfield(
        default=None,  # AppBorderRatio('TAIMEE:2.2,3.2') #dict[str, list[float]] = field( default_factory=lambda:{"taimee":[2.2,3.2]})
    ...
    glob_max: int = argfield(
        default=100, help="Pick up files max. count found in glob pattern"
    )
    show: bool = argfield(default=False, help="Show images to check")
    ...
    area_param_dir: Optional[Path] = argfield(
        help="Screenshot image area parameter config file directory"
    )
    area_param_name_list: list[str] = argfield(
        default=[m.name for m in AREA_PARAM_NAME],
        help="Screenshot image area parameter name list",
    )
    area_param_file: Path = argfield(
        default="image-area-param.ini",
        help='Screenshot image area parameter config file: format as INI with ".ini" extention): in [image_area_param.<app>] section, items as "<area_name>=[<p1>,<p2>,<p3>,<p4>]" (e.g. "heading=[0,106,196,-1]") ',
    )
    ocr_filter_sqlite_db_name: Path = argfield(
        default="ocr-filter.db",
        help="SQLite DB file is going to be created in `image_dir` directory if not exists.",
    )  # (yyyy is like 2025)')
    data_year: int = argfield(
        default=0,
        help="Year of data (like -1, 0, 2025, ...). 0 means current year, negative value is difference from current year (like -1 means last year), positive value means a.d. year number (like 2025). If this value is larger than current year, an exception might be raised.",
    )
    data_month: int = argfield(
        default=0,
        help="Month of data (like -1, 0, 1, 2, ...). 0 means current month, negative value is difference from current month (like -1 means last month), positive value means month number (1: Jan, 2: Feb, ...). If this value is larger than current month, data's date is treated as the last year.",
    )
    show_ocr_area: bool = argfield(
        default=False, help="Show every area before to commit OCR"
    )
    exclude_area_param_set: Optional[set[str]] = argfield(
        help="Exclude a set of image area parameter names"
    )
    ...
    def glob_files(self):
        for ext in self.image_ext_set:
            ...
            self.files += [
                str(p)
                for p in (
                    Path(self.image_dir).rglob(
                        glob_pattern,
                        case_sensitive=self.case_sensitive,
                        recurse_symlinks=self.recurse_symlinks,
                    )
                    if self.rglob
                    else Path(self.image_dir).glob(
                        glob_pattern,
                        case_sensitive=self.case_sensitive,
                        recurse_symlinks=self.recurse_symlinks,
                    )
...
def main_settings_toml_lines(Settings: Type[Settings] = MainSettings) -> Iterator[str]:
    ...
    for line in (
        Binder(Settings()).format_toml_template()
    ):  # Need to generate an instance to get default values of default factory
        yield (line)
...
def get_toml_path(fullpath: str | None, replacement_chars: str | None = "_-") -> Path:
    ...
    if not replacement_chars:
        ...
        toml = node.parent / (
            node.stem.replace(replacement_chars[0], replacement_chars[1]) + ".toml"
        )
    else:
...
def load_main_settings(
    fullpath: Path | str,
    settings_class: Type[MainSettings] = MainSettings,
    table: str = "",
) -> MainSettings:
    """Load the TOML file and return the Settings instance of Binder"""
    with Path(fullpath).open("rb") as f:
...
def search_settings_file(
    script_fullpath: Path | str = Path(__file__), replace=("_", "-")
) -> Path:
    ...
    main_settings_file = script_dir / (
        toml_name := (
            Path(script_fullpath).stem.replace(replace[0], replace[1]) + ".toml"
        )
    )
    found = False
...
def load_merged_settings(
    by_file_settings: MainSettings,
    main_settings_class=MainSettings,
    sub_settings_class=MainSettings,
) -> MainSettings:
    ...
    file_main_diff = DeepDiff(
        (by_file_settings_dict := asdict(by_file_settings)),
    ...
    for key in file_main_diff.affected_root_keys:
        assert main_merged_settings[key] == by_file_settings_dict[key], (
            f"Key {key} has different values: {main_merged_settings[key]} != {by_file_settings_dict[key]}"
        )
    sub_settings = sub_settings_class()
    parser = ArgumentParser()
    parser.add_arguments(
        sub_settings_class, dest="settings"
    )  # from command line param.
    args = parser.parse_args()
    args_sub_diff = DeepDiff(
        (args_settings_dict := asdict(args.settings)),
        (sub_settings_dict := asdict(sub_settings)),
    )
    affected_args_sub_diff = {
        k: v
        for k, v in args_settings_dict.items()
        if k in args_sub_diff.affected_root_keys
    }
    affected_args_main_diff = {
        k: v
        for k, v in affected_args_sub_diff.items()
        if k in main_settings_dict.keys()
    }
    main_merged_settings = am.merge(main_merged_settings, affected_args_main_diff)
    main_merged_diff = {
        k: v
        for k, v in main_merged_settings.items()
        if k in file_main_diff.affected_root_keys
        or k in args_sub_diff.affected_root_keys
    }

    sub_merged_settings = am.merge(sub_settings_dict, args_settings_dict)
    for key in args_sub_diff.affected_root_keys:
        assert sub_merged_settings[key] == args_settings_dict[key], (
            f"Key {key} has different values: {sub_merged_settings[key]} != {args_settings_dict[key]}"
        )
    sub_merged_diff = {
        k: v
        for k, v in sub_merged_settings.items()
        if k in args_sub_diff.affected_root_keys
    }
    return sub_settings_class(**main_merged_diff, **sub_merged_diff)


def load_merged_settings_no_deep_merge(
    main_settings_file: Path | str,
    main_settings_class=MainSettings,
    sub_settings_class=MainSettings,
    file_main_diff_list: list[str] | None = None,
    args_sub_diff_list: list[str] | None = None,
) -> MainSettings:
    """Load and merge the main settings with the sub settings(descendent of main class: 'sub' is broader than 'main') from settings file(in TOML format, 'main' settings range) and command line parameters('sub' settings range)"""
...
def load_main_settings_safely(
    ...
    try:
        toml_path = (
            search_settings_file(file, replace)
            if search_file
            else Path(file).with_name(
                Path(file).stem.replace(replace[0], replace[1]) + ".toml"
            )
        )
        ...
        logger.error(
            "TOML configuration file[%s] is malformed or contains invalid syntax: %s",
            toml_path,
            e,
        )
        raise
...
if __name__ == "__main__":
    ...
    def run2(app: str):
        print(
            f"Running main settings for app[{app.__class__}]: main_settings.app={app}"
        )
