...
argspec assumes normal field is positional"""
...
def main_settings_from_dict(toml_dict: dict[str, Any]) -> MainSettings:
	return Binder(MainSettings).bind(toml_dict)
def every_toml_line(bound=MainSettings())-> Iterator[str]:
	from dataclass_binder import Binder
	for line in Binder(bound).format_toml_template(): # Need to generate an instance to get default values of default factory
		yield(line)
...
if __name__ == '__main__':
	...
	bound = MainSettings()
	print('\n'.join(every_toml_line(bound)))
	print('-' * 80)
	args = bound.from_argv()
	print(args)