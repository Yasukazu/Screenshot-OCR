...
class MainSettings(AppSettings):
	...
	def __post_init__(self):
		super().__post_init__()
		# if self.app is not None: self.app_to_suffixes[self.app] = self.app_name_to_suffix[self.app]
	...
	@classmethod
	def simple_load(cls, config_file: Path|str) -> 'MainSettings':
		"""Load the YAML file and return the Settings instance"""
		from simple_parsing import parse as simple_parse
		settings = simple_parse(config_class=cls, config_path=config_file)#, add_config_path_arg
		settings.app_to_suffixes |= cls().app_to_suffixes # add default values
		return settings
...
def load_merged_settings(main_settings_class = MainSettings, config_path='app-config.yaml') -> MainSettings:
	"""Load and merge the main settings with settings file(in YAML format) and command line parameters('sub' settings range).
	`app_to_suffixes` dict. is merged.
	By `simple-parsing` module, parse the settings file and command line parameters."""
	from simple_parsing import parse as simple_parse
	app_settings: AppSettings = simple_parse(config_class=main_settings_class, config_path=config_path)#, add_config_path_arg
	app_settings.app_to_suffixes |= AppSettings().app_to_suffixes # add default values
	return app_settings

def load_merged_settings_deep(by_file_settings: MainSettings, main_settings_class = MainSettings, sub_settings_class = MainSettings) -> MainSettings:
...
if __name__ == '__main__':
	...
	@classmethod
	def bind_load(cls, fullpath: Path|str, table: str = "") -> 'MainSettings':
		"""Load the TOML file and return the Settings instance of Binder"""
		with Path(fullpath).open("rb") as f:
			config = tomllib.load(f)
...
from returns.result import safe
@safe
def load_main_settings_safely(file: str|Path = __file__, loader=MainSettings.simple_load, replace_char_pair: Sequence[str] = ('_', '-'), search_file=False) -> MainSettings:
	"""Load main settings(with exception handlings as messages: FileNotFoundError, OSError, YAMLError, KeyError, ValueError) from a configuration file, the name is replaced the filename of the script as underscore(_) to hypen(-).
	Returns: (Settings, Path)"""
	from yaml import YAMLError
	try:
		config_path = search_settings_file(file, replace_char_pair) if search_file else Path(file).with_name(Path(file).stem.replace(replace_char_pair[0], replace_char_pair[1]) + '.yaml')
		main_settings = loader(config_path)
	except FileNotFoundError as e:
		logger.error("Configuration file not found: %s", e)
		raise
	except OSError as e:
		logger.error("File system error accessing configuration file: %s", e)
		raise
	except YAMLError as e:
		logger.error("Configuration file[%s] is malformed or contains invalid syntax: %s", config_path, e)
		raise
	except (KeyError, ValueError) as e:
		logger.error("Configuration error in main settings: %s", e)
		raise
