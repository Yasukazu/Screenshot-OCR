...
### scan left-top area for (white) area
	...
	BLUR_SIZE = 47
	heading_area_width = heading_area.shape[1]
	H_BLUR_RATIO = 2
	blurred = cv2.blur(heading_area, ( heading_area_width // H_BLUR_RATIO, 1))
	gaussian_blurred = cv2.GaussianBlur(heading_area, (BLUR_SIZE, BLUR_SIZE // H_BLUR_RATIO), 0)
	# med_blurred = cv2.medianBlur(heading_area, 13)
	SUBPLOT_SIZE = 3
	fig, ax = plt.subplots(SUBPLOT_SIZE, 1)
	for r in range(SUBPLOT_SIZE):
		ax[r].invert_yaxis()
		ax[r].xaxis.tick_top()
	ax[0].imshow(heading_area)
	ax[1].imshow(blurred)
	ax[2].imshow(gaussian_blurred)
	plt.show()
	pass
	# frame = get_frame(heading_area)
...
def find_runs(x: Sequence[int]|np.ndarray)->tuple[np.ndarray, np.ndarray, np.ndarray]:
