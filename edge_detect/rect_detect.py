...
def main(filename: str, cutoff=0.3, BGR='B'):
    ...
    blurred = cv2.GaussianBlur(luminosity_result, (57, 57), 3) # medianBlur

    # Binarize
    #thresh_binarized = cv2.threshold(blurred, 200, 255,cv2.THRESH_BINARY)
    #thresh_binarized = cv2.adaptiveThreshold(blurred, 255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,\ cv2.THRESH_BINARY,11,2)
